...
public class ENV  extends Environment{
	...
	*/
    ...
    public boolean executeAction(String ag, Structure action) {
		...
		if (action.getFunctor().equals(hd)) {
        	/*Calculating HummingDistance */
			int distance=getHummingDistance(action.getTerm(0).toString(),action.getTerm(1).toString());
        	
			if(debugMode)
			logger.info("HummingDistance btw 2 agents term1: "+action.getTerm(0)+" term2:"+action.getTerm(1)+" D="+distance);
		...
		if (action.getFunctor().equals(hde)) {
			...
			if(debugMode)
			logger.info("HummingDistance btw agent and env term1: "+action.getTerm(0)+" term2:"+realString+" D="+distance);
			
			double punteggio=Math.random()*realString.length();
			if(debugMode)
			logger.info("Prop of collaboration "+punteggio+" should > distance "+distance);
		...
		if (action.getFunctor().equals(gns)) {
			...
			if(debugMode)
			logger.info("GENERATE STRING FROM leader string: "+action.getTerm(0)+" AND follower string:"+action.getTerm(1)+" TO new string"+newString);
		}
	...
	synchronized void incrementReadyAgent(){
		readyAgent++;
		//logger.info("agent num: "+readyAgent+"/"+nAgents);
		if(debugMode)
	...
	*/
	...
	void printAgentsPossession(){
		...
		logger.info("=============== GOAL ORIENTED VERSION END AGENTE REPORT ==============");
